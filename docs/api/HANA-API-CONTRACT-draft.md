...
## مرز درآمد بازارگاه و سامانه لجستیک (ADR-044)

طبق [ADR-044](../adr/ADR-044-SEPARATE-HANA-FIXED-INVOICE-FEE-FROM-LOGISTICS-COURIER-REVENUE.md)، کارمزد فروش حنا **مبلغ ثابت به‌ازای هر فاکتور** و مستقل از ارزش درصدی سفارش است؛ مقدار کارمزد از تعرفه نسخه‌دار و مصوب دریافت شود، نه عدد hard-code یا درصد سفارش. «۱۰٪ از پیک» **درآمد سامانه/کسب‌وکار لجستیک** است و به درآمد حنا اضافه نشود. نام/مسیر endpointهای نهایی بعداً در OpenAPI تثبیت می‌شوند، ولی فیلدهای منطقی `marketplaceFixedInvoiceFeeIrr`, `marketplaceFeePolicyVersion`, `deliveryTotalIrr`, `sellerDeliveryShareIrr` و `logisticsSettlementReference` (در صورت نیاز تطبیق) از هم مجزا باشند؛ منطق/مبلغ درآمد مستقل لجستیک در API درآمد بازارگاه وارد نشود.

## چرخه تسویه پایان هر روز کاری همه فروشگاه‌ها

طبق [ADR-038](../adr/ADR-038-ALL-SELLERS-SETTLED-END-OF-EACH-WORKDAY.md)، تسویه مبالغ **واجد شرایط همه فروشگاه‌ها در پایان هر روز کاری حنا** انجام می‌شود؛ عبارت تاریخی «یک روز بعد از پرداخت/تحویل» در ADR-008 دیگر مبنای eligibility نیست. سرویس مالی باید `settlementCycleId`، `businessDate`، `cycleState`، `cycleClosedAt`، وضعیت payment attempt/تطبیق و فهرست فاکتورهای انتخاب‌شده و مستثناشده از batch را قابل پیگیری کند. طبق [ADR-039](../adr/ADR-039-THURSDAY-IS-HANA-SELLER-SETTLEMENT-BUSINESS-DAY.md) و [ADR-040](../adr/ADR-040-FRIDAY-IS-HANA-SELLER-SETTLEMENT-BUSINESS-DAY.md)، پنجشنبه و جمعه باید هر دو روز کاری عادی batch تسویه شناخته شوند؛ طبق [ADR-041](../adr/ADR-041-SETTLE-ALL-SELLERS-ON-OFFICIAL-HOLIDAYS.md) **تعطیلات رسمی نیز بدون حذف از چرخه، در پایان همان روز پردازش می‌شوند**. همه روزهای تقویمی چرخه دارند؛ طبق [ADR-042](../adr/ADR-042-AUTOMATIC-SELLER-SETTLEMENT-AT-MIDNIGHT-BANK-CYCLE-DEPENDENT.md) اجرای خودکار حنا **هر شب ۰۰:۰۰ نیمه‌شب** است و چرخه در مرز ورود به تاریخ جدید برای روز پایان‌یافته ثبت می‌شود؛ منطقه زمانی مرجع طبق [ADR-043](../adr/ADR-043-MIDNIGHT-SETTLEMENT-IN-IRAN-TIMEZONE.md) **`Asia/Tehran`** است؛ `businessDate` تاریخ روز محلی ایران پایان‌یافته و timestampهای ثبت‌شده UTC هستند. زمان وصول واقعی بر اساس چرخه بانک هنوز policy/provider-dependent است و نباید در API حدس زده شود.
