...
## لغو سفارش سوپرمارکتی پیش از شروع آماده‌سازی

طبق [ADR-010](../adr/ADR-010-CUSTOMER-CANCEL-BEFORE-PREPARATION.md)، خریدار می‌تواند پیش از شروع واقعی آماده‌سازی فروشنده درخواست لغو کند؛ endpoint پیشنهادی `POST /orders/{id}/cancel` با کنترل مالکیت، `Idempotency-Key` و بررسی اتمیک state/version. `SELLER_CONFIRMED` لزوماً به معنی `PREPARING` نیست. API باید تعارض لغو با شروع هم‌زمان آماده‌سازی را با وضعیت واقعی پاسخ دهد؛ درخواست لغو تکراری منجر به دو استرداد یا دو رویداد لجستیک نشود. زمان/مقدار بازپرداخت و سیاست بعد از شروع آماده‌سازی منتظر تصویب‌اند.
