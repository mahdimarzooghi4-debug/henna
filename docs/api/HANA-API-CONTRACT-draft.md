...
درخواست ساخت `POST /quotes` باید به شناسه سبد، فروشنده/پیشنهاد منتخب، نشانی تحویل، روش ارائه و حالت خرید اشاره کند. پاسخ پیشنهادی:
...
پس از تصویب قواعد D01–D10 در سند V1.0: OpenAPI 3.1 برای API، JSON schemaهای درخواست/پاسخ، AsyncAPI یا schema رخداد، auth matrix و contract tests ثبت شوند. تا آن زمان این قرارداد راهنمای طراحی است.
