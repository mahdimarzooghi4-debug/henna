...
طبق [ADR-038](../adr/ADR-038-ALL-SELLERS-SETTLED-END-OF-EACH-WORKDAY.md)، تسویه مبالغ **واجد شرایط همه فروشگاه‌ها در پایان هر روز کاری حنا** انجام می‌شود؛ عبارت تاریخی «یک روز بعد از پرداخت/تحویل» در ADR-008 دیگر مبنای eligibility نیست. سرویس مالی باید `settlementCycleId`، `businessDate`، `cycleState`، `cycleClosedAt`، وضعیت payment attempt/تطبیق و فهرست فاکتورهای انتخاب‌شده و مستثناشده از batch را قابل پیگیری کند. طبق [ADR-039](../adr/ADR-039-THURSDAY-IS-HANA-SELLER-SETTLEMENT-BUSINESS-DAY.md)، پنجشنبه باید روز کاری عادیِ batch تسویه شناخته شود؛ جمعه، پنجشنبه مصادف با تعطیل رسمی، ساعت/منطقه زمانی و سایر تعطیلات هنوز policy بازند و نباید در API حدس زده شوند.
