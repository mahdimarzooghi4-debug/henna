...
**قانون قطعی سفارش:** [ADR-001](../adr/ADR-001-SINGLE-SELLER-ORDER.md) تصویب می‌کند که هر سفارش و هر checkout دقیقاً یک فروشنده دارد؛ سفارش چندفروشنده‌ای پیاده نمی‌شود. خرید از فروشنده دیگر سفارش مستقل دارد.
...
**قرارداد Quote:** `quoteId`, `sellerId`, `deliveryAddressId`, `cityId`, `items[]`, `itemsTotalIrr`, `deliveryFeeIrr`, `discountIrr`, `creditAppliedIrr`, `payableIrr`, `expiresAt`, `offerVersion`, `policyVersion`, `availableFulfillmentModes[]`. تمام مبالغ در ریال صحیح و با قواعد تبدیل/گردکردن تصویب‌شده؛ تومان صرفاً نمایش.
...
| F04 | سبد | چند قلم، مقدار، حذف، قیمت مرجع و سبد پایدار |
| F05 | مقایسه | قیمت کامل، فاصله صحیح، موجودی، ارسال و عدم انتخاب نامعتبر |
| F06 | quote/Checkout | قیمت و ارسال هماهنگ، انقضا و اعلام تغییر قبل پرداخت |
...
| F10 | پرداخت | success/failure/unknown، webhook تکراری، استعلام و reconciliation |
...
| F17 | مالی/تسویه | ledger، مبالغ قابل تسویه، قرارداد مصوب، بازگشت وجه و تطبیق |
...
| D02 | تعریف مدل مقایسه فروشگاه، محدوده، هزینه ارسال، pick-up و ظرفیت خدمت | Geo/Quote/UX |
| D03 | پرداخت در محل و سیاست لغو/مرجوعی/جبران/تسویه | Finance/Operations |
...
**D01 طبق ADR-001 تصویب شده است. پیش از کدنویسی می‌توان اسکلت، UI، mock در dev/stage و قراردادهای اولیه را ساخت؛ اما پیش از freeze دامنه و انتشار عمومی شهر اول، D02–D10 باید به تصمیم امضاشده/ADR با مالک و تاریخ تبدیل شوند.** این سند معماری را کامل می‌کند، ولی سیاست‌های کسب‌وکار را بدون مالک جعل نمی‌کند.
