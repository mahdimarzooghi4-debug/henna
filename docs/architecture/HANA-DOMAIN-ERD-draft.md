...
برای هر اجرای چرخه و دستور payout، `(businessDate, sellerId, cycleType)` یا کلید یکتای هم‌ارز، `payoutAttemptId`, `providerReference` و وضعیت جداگانه `PAYOUT_SUBMITTED | PAYOUT_UNKNOWN | PAID` نیاز است. اعتبار `PAID` به موفقیت واقعی/تطبیق بانکی وابسته است. طبق [ADR-039](../adr/ADR-039-THURSDAY-IS-HANA-SELLER-SETTLEMENT-BUSINESS-DAY.md)، پنجشنبه جزو روزهای کاری معمول تسویه است؛ جمعه، تعطیلات رسمی، ساعت دقیق روز کاری و مبدأ cut-off هنوز سیاست باز هستند و از مدل timestamp سفارش استنتاج نمی‌شوند. `calendarPolicyVersion` باید این تصمیم را در batchهای پنجشنبه قابل‌بازتولید کند. طرح و کلیدهای فوق پیشنهاد migration فنی‌اند و نه schema اجراشده.
