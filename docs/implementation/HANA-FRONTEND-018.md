...
- روی کارت منتشرشدهٔ فهرست، لینک قابل‌دسترس به `/products/{id}` واقعی اضافه شد. مسیر جزئیات یک صفحهٔ مستقل Next است: در بازکردن مستقیم URL، برگشت به فهرست، refresh، وضعیت‌های API همگی کار می‌کنند؛ حفظ جست‌وجو/فیلتر/شمارهٔ صفحهٔ قبلی هنگام بازگشت از جزئیات در [Frontend 019](HANA-FRONTEND-019.md) به URL قابل‌اشتراک اضافه شد.
