...
**ادعای این مرحله ساخت APK/iOS release یا انجام آزمون تحویل لینک روی دستگاه نیست.** برای App/Universal Links عادیِ `https://` باید دامنهٔ production، `assetlinks.json`/AASA و امضای نسخهٔ نصب‌شده با دسترسی اپراتوری تهیه و آزمایش شوند. Expo Go/Simulator/Android physical دستگاه و دریافت واقعی پیامک هنوز خارج از شواهد این PR هستند. هیچ تغییر قیمت/موجودی/سبد/پرداخت انجام نشده است.
