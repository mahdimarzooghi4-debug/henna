...
`200 items: []` حالت خالی واقعی است؛ `503`، قطعی و پاسخ نامعتبر همیشه unavailable و «تلاش دوباره» هستند. لیست قبلی در هنگام شروع query تازه دوباره نمایش داده نمی‌شود. categories و products به‌شکل مستقل خطا/تلاش مجدد دارند. محصول card فقط `name/kind/description` دارد و لینک جزئیات ندارد (فریم جزئیات هنوز مصوب نیست). قیمت، موجودی، شهر سرویس‌پذیر، سبد، سفارش و پرداخت از قرارداد public قابل استنباط نیست.
