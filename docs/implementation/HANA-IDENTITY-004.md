...
تست واقعی PostgreSQL برای رد sender، نتیجه نامعلوم، cooldown بین DbContextها، باطل‌شدن کد قبلی و دو صدور هم‌زمان؛ تست قبلی تأیید کد نیز اکنون چالش `PENDING/FAILED` را رد می‌کند. CI migration جدید را نصب و دوباره اجرا می‌کند و buildهای وب و Expo را حفظ می‌کند. بررسی تست‌های موفق از GitHub Actions لازم است. **نتیجه ثبت‌شده:** [اجرای موفق CI مرحله 004](https://github.com/mahdimarzooghi4-debug/henna/actions/runs/35517717346) هر سه job بک‌اند، وب و موبایل را گذراند: ۱۴ آزمون دامنه و ۱۸ آزمون زیرساخت؛ بدون شکست.
