...
## پشته تصویب‌شده و زمان آغاز توسعه

طبق [ADR-045](../adr/ADR-045-HANA-TECHNOLOGY-STACK-ASPNET-CORE-DOTNET-10-LTS.md)، **هسته بک‌اند حنا C# / ASP.NET Core / .NET 10 LTS** با معماری Modular Monolith است؛ PostgreSQL، EF Core، SQL مستقیم کنترل‌شده در موارد لازم، .NET Worker Service + outbox/صف پایدار، Redis فقط برای cache، وب Next.js/TypeScript و اپ خریدار React Native/Expo/TypeScript هستند. قرارداد OpenAPI مرز تبادل بک‌اند و کلاینت‌هاست؛ لجستیک کسب‌وکار مستقل و اتصال API آن پابرجاست. NestJS پیشنهاد قدیمی بوده و برای شروع backend مبنا نیست.

**این ثبت فقط تصمیم فناوری و آماده‌سازی نقشه اجراست؛ توسعه/اسکلت پروژه تا دستور صریح بعدی مالک محصول آغاز نمی‌شود.** هنگام شروع، اول solution و migration/test/CI قابل اجرای .NET و کلاینت وب، سپس ماژول‌های هویت، فروشگاه، سفارش و مالی ساخته شوند.
...
| عملیات مشترک | پشتیبانی، اعلان، CMS، داشبورد، **بخش لجستیک در ادمین حنا با اتصال API سرویس لجستیکِ تأمین‌شده از بیرون و طراحی صفحه‌های آن در فیگما هنگام ساخت بخش طبق ADR-046**، گزارش | داده دامنه و [ADR-004](../adr/ADR-004-HANA-LOGISTICS-INDEPENDENT-INTEGRATION.md)/[ADR-005](../adr/ADR-005-PICKUP-CUSTOMER-DELIVERY-ASSIGNMENT-LOGISTICS.md) |
...
جریان‌های موازی UI می‌توانند زودتر شروع شوند، اما Mock به محیط انتشار راه ندارد. مرجع صفحه‌های دارای طرح مصوب، موجودی Figma است؛ طبق [ADR-046](../adr/ADR-046-ADMIN-LOGISTICS-SECTION-EXTERNAL-SERVICE-FIGMA-DEFERRED.md) **بخش لجستیک در دامنه ادمین حنا قرار دارد ولی فریم آن هنگام طراحی/ساخت همان بخش به Figma اضافه می‌شود**؛ بدون فریم تأییدشده ادعای طراحی کامل نشود و این انتظار توسعه سایر بخش‌ها را متوقف نکند. لجستیک عملیاتی از بیرون تأمین می‌شود، نه در backend بازارگاه؛ صفحات فروشندگی موبایل خارج از scope و خرید حقوقی مصرف‌کننده داخل scope است.

## تفکیک درآمد حنا و لجستیک برای اجرا (ADR-044)

- درآمد فروش بازارگاه حنا طبق [ADR-044](../adr/ADR-044-SEPARATE-HANA-FIXED-INVOICE-FEE-FROM-LOGISTICS-COURIER-REVENUE.md) **مبلغ ثابت به‌ازای هر فاکتور** است، نه درصدی از ارزش خرید؛ مقدار دقیق تعرفه و قرارداد سهم صندوق به‌صورت جداگانه تثبیت شوند.
- «۱۰٪ از پیک» **درآمد کسب‌وکار مستقل لجستیک** است؛ به‌عنوان درآمد بازارگاه حنا در داشبورد، بودجه، مدل مالی، ledger یا API گزارش نشود. یکپارچه‌سازی API حمل طبق ADR-004 و کسر سهم ارسال فروشنده طبق ADR-007 پابرجاست اما منشأ درآمد بازارگاه نمی‌سازد.
- سناریوی QA: دو فاکتور با مبلغ خرید متفاوت ولی تعرفه یکسان، کارمزد حنای یکسان دارند؛ سهم پیک لجستیک به درآمد حنا اضافه نمی‌شود؛ کسور ارسال با کارمزد ثابت قاطی نشوند.
